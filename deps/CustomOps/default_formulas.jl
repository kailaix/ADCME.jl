"sparse_diag_mat_mul" => ("SparseMatMul", "libSparseMatMul", "sparse_sparse_mat_mul", false),
"sparse_assembler" => ("SparseAccumulate", "libSparseAccumulator", "", false),
"sparse_mat_mul" => ("SparseMatMul", "libSparseMatMul", "sparse_sparse_mat_mul", false),
"sparse_least_square" => ("SparseLeastSquare", "libSparseLeastSquare", "sparse_least_square", true),
"sparse_indexing" => ("SparseIndexing", "libSparseIndexing", "sparse_indexing", true),
"diag_sparse_mat_mul" => ("SparseMatMul", "libSparseMatMul", "sparse_sparse_mat_mul", false),
"sparse_solver" => ("SparseSolver", "libSparseSolver", "sparse_solver", true),
"sinkhorn_knopp" => ("OT/SinkhornKnopp", "libSinkhornKnopp", "sinkhorn_knopp", true),
"sparse_concate" => ("SparseConcate", "libSparseConcate", "sparse_concate", true),
"sparse_scatter_update" => ("SparseScatterUpdate", "libSparseScatterUpdate", "sparse_scatter_update", true),
"sparse_factorization" => ("SparseFactorizationSolve", "libfactorization", "sparse_factorization", false),
"sparse_solve" => ("SparseFactorizationSolve", "libSolve", "solve", true)

