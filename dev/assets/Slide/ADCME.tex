...
\begin{itemize}
	...
	{\footnotesize $$ J = f_4(\mathbf{u}_1, \mathbf{u}_2, \mathbf{u}_3, \mathbf{u}_4) + \bm{\lambda}^{T}_{2}(f_1(\mathbf{u}_1, \bm {\theta}) - \mathbf{u}_2) + \bm{\lambda}^T_{3}(f_2(\mathbf{u}_2, \bm {\theta}) - \mathbf{u}_3) + \bm{\lambda}^T_{4}(f_3(\mathbf{u}_3, \bm {\theta}) - \mathbf{u}_4)$$}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item A more efficient way is to apply the \textcolor{red}{implicit function theorem}. For our example, $F(x,y)=x^3-(y^3+y)=0$, treat $y$ as a function of $x$ and take the derivative on both sides
...
\begin{frame}
	...
	\begin{figure}[hbt]
  \includegraphics[width=1.0\textwidth]{../summary.pdf}
\end{figure}
